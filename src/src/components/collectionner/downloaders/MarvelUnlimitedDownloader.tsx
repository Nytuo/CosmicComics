...
import {
  clearSavedMarvelUnlimitedCookies,
  closeMarvelUnlimitedAuth,
  downloadMarvelUnlimitedComic,
  getMarvelComicDetails,
  getMarvelSeriesComics,
  getMarvelUnlimitedCookies,
  getMarvelUnlimitedNewComics,
  loadNewComicsCache,
  saveNewComicsCache,
  hasSavedMarvelUnlimitedCookies,
  loadSavedMarvelUnlimitedCookies,
  openMarvelUnlimitedAuth,
  saveMarvelImages,
  searchMarvelUnlimitedComics,
  searchMarvelUnlimitedSeries,
} from '@/API/TauriAPI.ts';
import {
  IMarvelUnlimitedComic,
  IMarvelUnlimitedCookies,
  IMarvelUnlimitedDownloadProgress,
  IMarvelUnlimitedSeries,
} from '@/interfaces/IMarvelUnlimited.ts';
...
export default function MarvelUnlimitedDownloader({
  CosmicComicsTemp,
}: {
  ...
  const [comicResults, setComicResults] = useState<IMarvelUnlimitedComic[]>([]);
  const [seriesResults, setSeriesResults] = useState<IMarvelUnlimitedSeries[]>(
    []
  );
  const [downloadingComics, setDownloadingComics] = useState<
    Map<string, IMarvelUnlimitedDownloadProgress>
  >(new Map());
  const [selectedComic, setSelectedComic] =
    useState<IMarvelUnlimitedComic | null>(null);
  const [selectedSeries, setSelectedSeries] =
    useState<IMarvelUnlimitedSeries | null>(null);
  const [seriesComics, setSeriesComics] = useState<IMarvelUnlimitedComic[]>([]);
  const [loadingDetails, setLoadingDetails] = useState(false);
  const [newComics, setNewComics] = useState<IMarvelUnlimitedComic[]>([]);
  ...
  useEffect(() => {
    const loadCookies = async () => {
      try {
        const hasCookies = (await hasSavedMarvelUnlimitedCookies()) as boolean;
        if (hasCookies) {
          Logger.info('Loading saved Marvel Unlimited cookies');
          const result =
            (await loadSavedMarvelUnlimitedCookies()) as unknown as IMarvelUnlimitedCookies;
  ...
  const handleSelectComic = async (comic: IMarvelUnlimitedComic) => {
    setLoadingDetails(true);
    try {
      const details = (await getMarvelComicDetails(
        comic.id,
        cookies
      )) as IMarvelUnlimitedComic;
  ...
  const handleSelectSeries = async (series: IMarvelUnlimitedSeries) => {
    setLoadingDetails(true);
    try {
      const comics = (await getMarvelSeriesComics(
        series.id,
        cookies
      )) as IMarvelUnlimitedComic[];
    ...
    const fetchNewComics = async () => {
      ...
      try {
        if (!newComicsCacheChecked) {
          const cached = (await loadNewComicsCache(86400)) as
            | IMarvelUnlimitedComic[]
            | null;
        ...
        const results = (await getMarvelUnlimitedNewComics(
          cookies
        )) as IMarvelUnlimitedComic[];
        setNewComics(results);
        saveNewComicsCache(results).catch((e) =>
  ...
  useEffect(() => {
    const unlisten = listen('marvel-images-captured', async (event: any) => {
      const { comic_id, images } = event.payload;
      Logger.info(`Received ${images.length} images for comic ${comic_id}`);

      try {
        if (!images || images.length === 0) {
          ToasterHandler(t('No images found in reader'), 'warning');
          return;
        }

        let comicTitle = t('unknown-comic');
        const downloadProgress = downloadingComics.get(comic_id);
        if (downloadProgress) {
          comicTitle = downloadProgress.comicTitle;
        } else {
          const comic = comicResults.find((c) => c.id === comic_id);
          if (comic) comicTitle = comic.title;
        }

        ToasterHandler(
          t('Downloading') + ` ${images.length} ` + t('pages'),
          'info'
        );
        await saveMarvelImages(comic_id, comicTitle, images, undefined);
        ToasterHandler(t('download-completed'), 'success');
      } catch (error) {
        Logger.error('Failed to download images: ' + JSON.stringify(error));
        ToasterHandler(t('Failed to download images'), 'error');
      }
    });
  ...
  }, [t, downloadingComics, comicResults]);

  useEffect(() => {
    const unlisten = listen('marvel-download-progress', (event: any) => {
      const progress = event.payload as IMarvelUnlimitedDownloadProgress;
      ...
      Logger.info(
        `Download progress for ${progress.comicId}: ${progress.status}`
      );
  ...
  const handleOpenAuth = async () => {
    try {
      ToasterHandler(t('opening-marvel-unlimited-login'), 'info');
      const windowLabel = await openMarvelUnlimitedAuth();
      setAuthWindowLabel(windowLabel as unknown as string);
      ToasterHandler(t('Please log in to Marvel Unlimited'), 'info');
    } catch (error) {
  ...
  const handleGetCookies = async () => {
    ...
    try {
      const result = (await getMarvelUnlimitedCookies(
        authWindowLabel
      )) as unknown as IMarvelUnlimitedCookies;
      ...
      if (result.authenticated) {
        ...
        await closeMarvelUnlimitedAuth(authWindowLabel);
        setAuthWindowLabel('');
      } else {
  ...
  const handleLogout = async () => {
    try {
      await clearSavedMarvelUnlimitedCookies();
      setCookies({});
  ...
  const handleSearch = async () => {
    ...
    try {
      if (tabValue === '0') {
        const results = (await searchMarvelUnlimitedComics(
          searchQuery,
          cookies
        )) as IMarvelUnlimitedComic[];
        ...
        const results = (await searchMarvelUnlimitedSeries(
          searchQuery,
          cookies
        )) as IMarvelUnlimitedSeries[];
  ...
  const handleDownloadComic = async (comic: IMarvelUnlimitedComic) => {
    try {
      ToasterHandler(t('Starting download for') + ` ${comic.title}...`, 'info');

      const progress: IMarvelUnlimitedDownloadProgress = {
        comicId: comic.id,
      ...
      const savePath = `${CosmicComicsTemp}/downloads/marvel_unlimited`;
      const result = (await downloadMarvelUnlimitedComic({
        comicId: comic.id,
        comicTitle: comic.title,
        cookies: cookies,
        savePath: savePath,
  ...
  const getDownloadButtonForComic = (comic: IMarvelUnlimitedComic) => {
  ...
  const insertComicIntoDB = async (
    comic: IMarvelUnlimitedComic,
    path: string
  ) => {
    try {
      await TauriAPI.insertMarvelUnlimitedBookToDB(comic, path);
  ...
  const transformComicForViewer = (comic: IMarvelUnlimitedComic) => {
    const allCreators = [
      ...(comic.creators || []),
      ...(comic.extendedCredits || []),
    ];

    let enhancedDescription = comic.description || '';
    if (comic.rating || comic.format || comic.upc || comic.price) {
      enhancedDescription += '\n\n';
      ...
      if (comic.upc) enhancedDescription += `UPC: ${comic.upc}\n`;
      if (comic.focDate) enhancedDescription += `FOC Date: ${comic.focDate}\n`;
    }
    ...
    return {
      ...
      provider_id: 5,
      provider_name: 'Marvel Unlimited',
      title: comic.title,
      ...
      extra: {
        urls: comic.seriesId
          ? [{ url: `https://www.marvel.com/comics/series/${comic.seriesId}` }]
          : [],
  ...
  const transformSeriesForViewer = (series: IMarvelUnlimitedSeries) => {
    return {
      id: series.id,
      ...
      {!authenticated ? (
        <div className="text-center py-8">
          <div className="rounded-lg border p-3 text-sm bg-blue-500/10 text-blue-500 mb-6">
            {t(
              'You need to authenticate with Marvel Unlimited to download comics'
            )}
          </div>
          <div className="flex flex-col items-center gap-3">
            <Button onClick={handleOpenAuth} size="lg">
              <LogIn className="mr-2 h-4 w-4" />
              {t('Open Marvel Unlimited Login')}
            </Button>
            {authWindowLabel && (
        ...
        <div>
          <div className="flex items-center justify-between gap-4 mb-4">
            <div className="rounded-lg border p-3 text-sm bg-green-500/10 text-green-500 flex-1">
              {t('Authenticated with Marvel Unlimited')} ✓
            </div>
            <Button
              variant="outline"
              size="sm"
            ...
            <TabsContent value="2">
              <div className="flex items-center justify-between mt-4 mb-3">
                <p className="text-sm text-muted-foreground">
                  {newComics.length > 0
                    ? t('Showing') +
                      ` ${newComics.length} ` +
                      t('new comics from Marvel Unlimited')
                    : t('No new comics from Marvel Unlimited')}
                </p>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              {loadingNewComics ? (
                <div className="flex flex-col items-center gap-3 py-10">
                  <Spinner size="lg" />
                  <p className="text-sm text-muted-foreground">
                    {t('loading-new-comics-from-marvel-unlimited')}
                  </p>
                </div>
              ) : newComics.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-6">
