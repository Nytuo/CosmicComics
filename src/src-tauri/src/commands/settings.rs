...
fn all_credential_definitions() -> Vec<CredentialDefinition> {
    vec![
        ...
        CredentialDefinition {
            key: "metron_api_key".into(),
            label: "Metron API Token".into(),
            description: "API token generated from your metron.cloud account page — takes precedence over username/password when set".into(),
            is_secret: true,
...
pub async fn load_credentials_into_state(state: &AppState) -> Result<(), String> {
    ...
    for (key, value) in &stored {
        ...
        match key.as_str() {
            ...
            "metron_api_key" => creds.metron_api_key = value.clone(),
