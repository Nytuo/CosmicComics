    ...
    fetch("http://" + domain + ":" + port + "/profile/logcheck/" + connected).then(function (response) {
        ...
        if (data === "false") {
            ...
            fetch("http://" + domain + ":" + port + "/config/getConfig/" + connected).then(function (response) {
                ...
                let currenttheme = GetElFromInforPath("theme", JSON.parse(data));
                console.log(currenttheme);
...
async function rematch(id, provider, type, old_id, isSeries = false) {
    if (isSeries) {
    ...
    await refreshMeta(id, provider, type);
}
...
async function AllForOne(W1, W2, A, ID) {
    let options = {
        method: "POST", headers: {
            ...
            "token": connected, "table": "Books", "column": A, "whereEl": ID, "value": true, "where": "ID_book"
        }, null, 2)
    };
    await fetch("http://" + domain + ":" + port + "/DB/update", options).then(async () => {
        options = {
            method: "POST", headers: {
                ...
                "token": connected, "table": "Books", "column": W1, "whereEl": ID, "value": false, "where": "ID_book"
            }, null, 2)
        };
        await fetch("http://" + domain + ":" + port + "/DB/update", options).then(async () => {
            options = {
